<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(item,index) in tradeInfo.userAddressList" :key="item.id">
<!--        selected-->
        <span class="username" :class="{selected:index==addressindex}" @click="addressindex=index">{{item.consignee}}</span>
        <p v-if="item.userAddress">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="detail in tradeInfo.detailArrayList" :key="detail.skuId" v-if="detail">
          <li>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo" v-if="tradeInfo.userAddressList">
        寄送至:
        <span>{{tradeInfo.userAddressList[addressindex].userAddress}}</span>
        收货人：<span>{{tradeInfo.userAddressList[addressindex].consignee}}</span>
        <span>{{tradeInfo.userAddressList[addressindex].phoneNum}}</span>
      </div>
    </div>
  ...
  export default {
    name: 'Trade',
      methods:{
          async sumitOrder () {
              // 准备参数数据
              const {orderComment,addressindex} = this
              const {tradeNo, detailArrayList} = this.tradeInfo
              const {consignee, phoneNum, userAddress} = this.tradeInfo.userAddressList[addressindex]
    ...
    computed:{
      ...mapState({
        tradeInfo:state=> state.orders.tradeInfo
      })
    },
    async mounted() {
      try {
